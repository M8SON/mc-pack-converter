...
## 15. Nine-slice GUI sprites rendered as missing textures above 1x

**Status:** FIXED 2026-08-01 — new `stages/gui_scaling.py` +
`data/gui_scaling.json` (generated by `tools/gen_gui_scaling.py`).
**Found by:** in-game testing — a Game Menu screenshot with every button
magenta. **72 of 170 corpus packs affected**, with zero warnings.

Modern Minecraft draws many GUI sprites with nine-slice scaling, declared in a
`.mcmeta` beside the texture:

```json
{"gui": {"scaling": {"type": "nine_slice", "width": 200, "height": 20, "border": 3}}}
```

Vanilla's `widget/button.png` is exactly 200×20 — the metadata describes the
texture it sits beside. A converted pack ships that sprite at **its own**
resolution (400×40 at 2×, 800×80 at 4×) and shipped no `.mcmeta` at all.

**Metadata is a separate resource.** Overriding the texture but not the
`.mcmeta` leaves vanilla's in force, still claiming 200×20, applied to a texture
several times that size. The sprite fails to resolve and draws the missing
texture. Nothing is wrong with the PNG, which is why no check caught it and why
the file survived every byte-level comparison.

33 vanilla sprites carry scaling metadata; the converter produces 18 of them —
buttons, sliders, tabs, toasts, advancement boxes, the recipe-book overlay and
the social-interactions background. Now each one the pack actually overrides
gets a `.mcmeta` whose pixel fields are scaled by the factor between the
produced texture and vanilla's declared size. Borders scale too, per-axis when
they are a per-edge dict (`toast/system` is `{left:17, top:30, right:4,
bottom:4}`).

**How it hid for so long:** it only affects packs above 1×, and only on screens
a PVP player rarely opens. The corpus never saw it — the texture is valid, the
path is right, the dimensions are self-consistent. It took someone opening the
Game Menu.

**Related, still open:** `hud/hotbar` has **no** scaling metadata in vanilla, so
this does not explain the magenta hotbar in the same pack. That remains
unresolved — the sprite is byte-identical to its source region, correctly named,
validly encoded, and its neighbours from the same atlas render.
